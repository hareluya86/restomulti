...
class Functions extends CApplicationComponent {
    ...
    public function merchantMenu() {
        $payment_list = array(
            'visible' => $this->hasMerchantAccess("payment-gateway"),
            'tag' => 'payment-gateway',
            'label' => '<i class="fa fa-usd"></i>' . Yii::t("default", 'Payment Gateway'),
            'itemOptions' => array('class' => ''),
            'items' => array(
                array(
                    'visible' => $this->hasMerchantAccess("pyp"),
                    'tag' => 'paypal',
                    'label' => '<i class="fa fa-paypal"></i>' . Yii::t("default", "Paypal"),
                    'url' => array('merchant/paypalSettings')
                ),
                array(
                    'visible' => $this->hasMerchantAccess("stp"),
                    'tag' => 'stripe',
                    'label' => '<i class="fa fa-paypal"></i>' . Yii::t("default", "Stripe"),
                    'url' => array('merchant/stripeSettings')
                ),
                array(
                    'visible' => $this->hasMerchantAccess("mcd"),
                    'tag' => 'mercadopago',
                    'label' => '<i class="fa fa-paypal"></i>' . Yii::t("default", "Mercadopago"),
                    'url' => array('merchant/mercadopagoSettings')
                ),
                array(
                    'visible' => $this->hasMerchantAccess("ide"),
                    'tag' => 'ide',
                    'label' => '<i class="fa fa-paypal"></i>' . Yii::t("default", "Sisow"),
                    'url' => array('merchant/sisowsettings')
                ),
                array(
                    'visible' => $this->hasMerchantAccess("payu"),
                    'tag' => 'payu',
                    'label' => '<i class="fa fa-paypal"></i>' . Yii::t("default", "PayUMoney"),
                    'url' => array('merchant/payumoneysettings')),
                array('visible' => $this->hasMerchantAccess("pys"), 
                    'tag' => 'pys', 
        ...
        return array(
            ...
            'items' => array(
                ...
                //$fax_menu,//we will deal with this bug later
    ...
    public function hasMerchantAccess($tag = '') {
        ...
        $tag_paymentgateway = array(
            ...
            'atz', 'obd', 'pyp', 'stp', 'mcd', 'ocr', 'btr', 'rzr'
        );
    ...
    public function updateOption($option_name = '', $option_value = '', $merchant_id = '') {
        ...
        if (is_array($rows) && count($rows) >= 1) {
            ...
            $res = $command->update('{{option}}', $params, 'option_name=:option_name and merchant_id=:merchant_id', array(
                ':option_name' => addslashes($option_name),
                ':merchant_id' => $merchant_id
                    )
            );
        ...
        if ($res = $DbExt->rst($stmt)) {
            foreach ($res as $val) {
                ...
                $addon_item = $this->addOnItemToArray(
                        $val['addon_item'], $multi_option, $multi_option_val, $two_flavors_position, $require_addon, $val['merchant_id']
                );
            ...
            if (is_array($arg1['items']) && count($arg1['items']) >= 1) {
                foreach ($arg1['items'] as $val) {
                    if (is_array($val['url']) && count($val['url']) >= 1) {
                        ...
                        $url = Yii::app()->request->baseUrl . $val['url'];
...
//$menu_html.=PointsProgram::frontMenu(false);
    ...
    public function adminPaymentList() {
        ...
        <?php if ($admin_enabled_paypal == ""): ?>
            ...
            <?php echo CHtml::radioButton('payment_opt', false, array('class' => "icheck payment_option", 'value' => "pyp"))
            ?> <?php echo Yii::t("default", "Paypal") ?>
                </div>   
        <?php endif; ?>

        <?php if ($admin_enabled_card == ""): ?>
                <div class="uk-form-row">
            <?php echo CHtml::radioButton('payment_opt', false, array('class' => "icheck payment_opt payment_option", 'value' => "ccr"))
            ?> <?php echo Yii::t("default", "Offline Credit Card") ?>
                </div>     
        <?php endif; ?>

        <?php if ($enabled_stripe == "yes"): ?>
                <div class="uk-form-row">
            <?php echo CHtml::radioButton('payment_opt', false, array('class' => "icheck payment_option", 'value' => "stp"))
            ?> <?php echo Yii::t("default", "Stripe") ?>
                </div>     
        <?php endif; ?>

        <?php if ($admin_mercado_enabled == "yes"): ?>
                <div class="uk-form-row">
            <?php echo CHtml::radioButton('payment_opt', false, array('class' => "icheck payment_option", 'value' => "mcd"))
            ?> <?php echo Yii::t("default", "Mercadopago") ?>
                </div>     
        <?php endif; ?>
        ...
        <?php if ($admin_sisow_enabled == "yes"): ?>
                <div class="uk-form-row">
            <?php echo CHtml::radioButton('payment_opt', false, array('class' => "icheck payment_option", 'value' => "ide"))
            ?> <?php echo Yii::t("default", "Sisow") ?>
                </div>     
        <?php endif; ?>         

        <?php if ($admin_payu_enabled == "yes"): ?>
                <div class="uk-form-row">
            <?php echo CHtml::radioButton('payment_opt', false, array('class' => "icheck payment_option", 'value' => "payu"))
            ?> <?php echo Yii::t("default", "PayUMoney") ?>
                </div>     
        <?php endif; ?>         

        <?php if ($admin_bankdeposit_enabled == "yes"): ?>
                <div class="uk-form-row">
            <?php echo CHtml::radioButton('payment_opt', false, array('class' => "icheck payment_option", 'value' => "obd"))
            ?> <?php echo Yii::t("default", "Bank Deposit") ?>
                </div>     
        <?php endif; ?>     

        <?php if ($admin_paysera_enabled == "yes"): ?>
                <div class="uk-form-row">
            <?php echo CHtml::radioButton('payment_opt', false, array('class' => "icheck payment_option", 'value' => "pys"))
            ?> <?php echo Yii::t("default", "Paysera") ?>
                </div>     
        <?php endif; ?>             

        <?php if ($admin_enabled_barclay == "yes"): ?>
                <div class="uk-form-row">
            <?php echo CHtml::radioButton('payment_opt', false, array('class' => "icheck payment_option", 'value' => "bcy"))
            ?> <?php echo Yii::t("default", "Barclay") ?>
                </div>     
        <?php endif; ?>             

        <?php if ($admin_enabled_epaybg == "yes"): ?>
                <div class="uk-form-row">
            <?php echo CHtml::radioButton('payment_opt', false, array('class' => "icheck payment_option", 'value' => "epy"))
            ?> <?php echo Yii::t("default", "EpayBg") ?>
                </div>     
        <?php endif; ?>             

        <?php if ($admin_enabled_autho == "yes"): ?>
                <div class="uk-form-row">
            <?php echo CHtml::radioButton('payment_opt', false, array('class' => "icheck payment_option", 'value' => "atz"))
            ?> <?php echo Yii::t("default", "Authorize.net") ?>
                </div>     
        <?php endif; ?>             
    ...
    public function SMSnotificationMerchant($merchant_id = '', $order_info = '', $data = '') {
        ...
        $sms_alert_message = $this->smarty("delivery-date", $this->translateDate($this->FormatDateTime($order_info['delivery_date'], false)), $sms_alert_message);
        $sms_alert_message = $this->smarty("delivery-time", $order_info['delivery_time'], $sms_alert_message);

        $sms_alert_message = $this->smarty("order-change", displayPrice(adminCurrencySymbol(), $this->standardPrettyFormat($order_info['order_change']))
                , $sms_alert_message);

        $sms_alert_message = $this->smarty("delivery-fee", displayPrice(adminCurrencySymbol(), $this->standardPrettyFormat($order_info['delivery_charge']))
                , $sms_alert_message);
        ...
        if (!empty($client_sms) && !empty($client_contact_phone)) {
            ...
            $client_sms = $this->smarty("delivery-date", $this->translateDate($this->FormatDateTime($order_info['delivery_date'], false)), $client_sms);
            $client_sms = $this->smarty("delivery-time", $order_info['delivery_time'], $client_sms);
            $client_sms = $this->smarty("order-change", displayPrice(adminCurrencySymbol(), $this->standardPrettyFormat($order_info['order_change']))
                    , $client_sms);
            $client_sms = $this->smarty("merchant-address", $order_info['merchant_address'], $client_sms);

            $client_sms = $this->smarty("delivery-fee", displayPrice(adminCurrencySymbol(), $this->standardPrettyFormat($order_info['delivery_charge']))
                    , $client_sms);
        }
        ...
        switch ($sms_provider) {
            ...
            case "msg91":
                ...
                $msg_resp = Msg91::sendSMS(
                                getOptionA('msg91_authkey'), $to, getOptionA('msg91_senderid'), $message, getOptionA('msg91_unicode'), $msg91_route
                );
                if ($msg_resp) {
                    $msg = "process";
                    $raw = $msg_resp;
    ...
    public function offlineBankDeposit($merchant = '', $data = '') {
        ...
        if (!empty($tpl)) {
            $tpl = $this->smarty('amount', $this->adminCurrencySymbol() . $this->standardPrettyFormat($package_price), $tpl);
