                ...
                <table class="table table-striped">
                    <tbody>	
                        ...
                        if ($data['payment_type'] == "pyp") {
                            ...
                            FunctionsV3::receiptTableRow('Paypal Transaction ID', isset($paypal_info['TRANSACTIONID']) ? $paypal_info['TRANSACTIONID'] : ''
                            );
                        }
                        ...
                        if ($data['payment_type'] == "ccr" || $data['payment_type'] == "ocr") {
                            FunctionsV3::receiptTableRow('Card #', $card = Yii::app()->functions->maskCardnumber($data['credit_card_number'])
                            );
                        }
                        ...
                        if ($data['trans_type'] == "delivery") {
                            ...
                            if (isset($data['delivery_time'])) {
                                if (!empty($data['delivery_time'])) {
                                    FunctionsV3::receiptTableRow('Delivery Time', Yii::app()->functions->timeFormat($data['delivery_time'], true));
                                }
                            }
                            if (isset($data['delivery_asap'])) {
                                if (!empty($data['delivery_asap'])) {
                                    FunctionsV3::receiptTableRow('Deliver ASAP', $data['delivery_asap'] == 1 ? t("Yes") : ''
                                    );
                                }
                            }
