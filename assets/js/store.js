...
jQuery(document).ready(function () {
    ...
    */
    ...
    $('form.frm-fooditem').submit(function(e){
        addToCart($(this));
        e.preventDefault();
    })
...
var addToCart = function (form) {
        ...
        var price = $("#"+formid+" .item_price:checked").length;
        console.debug(price);
        if (price <= 0) {
            if (!$("#"+formid+" #two_flavors").exists()) {
                uk_msg(js_lang.trans_29);
                $("#"+formid+" #price_wrap").focus();
                return;
            }
        }
        ...
        /** two flavors */
        if ($("#"+formid+" #two_flavors").exists()) {
            var sub_item = $("#"+formid+" .sub_item:checked").length;
            dump(sub_item);
            if (sub_item < 2) {
                uk_msg(js_lang.trans_46);
                return;
        ...
        /** check if addon is required */
        if ($("#"+formid+" .require_addon").exists()) {

            $("#"+formid+" .req_addon_msg").remove();

            var addon_err = '';
            $("#"+formid+" .require_addon").each(function () {
                if ($(this).val() == 2) {
                    var required_addon_id = $(this).data("id");
                    var required_addon_name = $(this).data("name");
                    dump(required_addon_id);
                    var required_addon_selected = $("#"+formid+" .sub_item_name_" + required_addon_id + ":checked").length;
                    dump(required_addon_selected);
                    if (required_addon_selected <= 0) {
                        addon_err = js_lang.trans_47 + " - " + required_addon_name + "<br/>";
                        $("#"+formid+" .require_addon_" + required_addon_id).after(
                                "<p class=\"req_addon_msg text-danger text-small top10\">" + addon_err + "</p>").fadeIn("slow");
                    }
                }
            });
